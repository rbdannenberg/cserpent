...
Any Obj::call(const Symbol& method, const Array &args,
              const Dictionary &kwargs) {
...
inline Any cs_class_get_name(Obj* self, const Array& args,
                             const Dictionary& kwargs) {
    check_dispatch("get_name", args, kwargs, 0, 0);
    return static_cast<Cs_class*>(self)->get_name();
}
inline Any cs_class_get_inst_slot_count(Obj* self, const Array& args,
                                        const Dictionary& kwargs) {
    check_dispatch("get_inst_slot_count", args, kwargs, 0, 0);
    return static_cast<Cs_class*>(self)->get_inst_slot_count();
}
inline Any cs_class_get_inst_any_slots(Obj* self, const Array& args,
                                       const Dictionary& kwargs) {
    check_dispatch("get_name", args, kwargs, 0, 0);
    return static_cast<Cs_class*>(self)->get_inst_any_slots();
}
inline Any cs_class_get_member_table(Obj* self, const Array& args,
                                     const Dictionary& kwargs) {
    check_dispatch("get_name", args, kwargs, 0, 0);
...
void main_init()
{
    cs_class_class = new Cs_class { Symbol {"Class"}, 5, 0b1,
                                    &cs_class_table };
